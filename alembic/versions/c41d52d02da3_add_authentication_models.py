...
def upgrade() -> None:
    ...
    op.create_index('idx_users_fs_uniquifier', 'users', ['fs_uniquifier'])
    ...
    op.create_table('webauthn',
        ...
        sa.Column('lastuse_datetime', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
    ...
    op.create_index('idx_webauthn_credential_id', 'webauthn', ['credential_id'])


def downgrade() -> None:
    """Downgrade schema - Remove authentication tables."""
    op.drop_index('idx_webauthn_credential_id', table_name='webauthn')
    op.drop_index('idx_webauthn_user_id', table_name='webauthn')
    op.drop_table('webauthn')
    op.drop_table('roles_users')
    op.drop_index('idx_users_fs_uniquifier', table_name='users')
