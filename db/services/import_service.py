...
from db.licensing import get_license_manager
...
class ConversationImportService:
    ...
    def import_json_data(self, data: Dict[str, Any]) -> ImportResult:
        ...
        try:
            ...
            # Check if format requires a license
            self._check_license_for_format(format_key, format_type)
    ...
    def import_docx_file(self, file_path: str, filename: str) -> ImportResult:
        ...
        try:
            ...
            # Check if DOCX format requires a license
            self._check_license_for_format('docx', 'DOCX')
    ...
    def _check_license_for_format(self, format_key: str, format_type: str) -> None:
        """Check if a format requires a license and validate it.
        
        Args:
            format_key: Format key in registry (e.g., 'chatgpt')
            format_type: Display name of format (e.g., 'ChatGPT')
            
        Raises:
            ValueError: If license is required but not valid
        """
        ...
        if capabilities.get('requires_license', False):
            license_manager = get_license_manager()
            
            # Map format to feature name
            feature_name = f"{format_key}_importer"
            ...
            if not license_manager.has_feature(feature_name):
                ...
                user_msg = get_user_friendly_error_message(error)
                raise ValueError(user_msg)
